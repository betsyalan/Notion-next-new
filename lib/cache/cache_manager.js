...
/**
 * 清空整条缓存链(redis/file/memory 中实现了 cleanCache 的 store)。
 * 生产环境只有 memory 生效,之前清理接口只清 file 导致清不掉真正生效的缓存。
 */
export async function cleanAllCaches() {
  const chain = getCacheChain()

  for (const { name, api } of chain) {
    if (typeof api.cleanCache !== 'function') continue
    try {
      await api.cleanCache()
      console.log(`[Cache] ${name} cleaned`)
    } catch (e) {
      console.warn(`[Cache] ${name} clean failed`, e.message)
    }
  }
}
