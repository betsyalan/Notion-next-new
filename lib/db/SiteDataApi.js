...
import { compressImage, mapImgUrl, resolveCustomEmojiIcon } from '@/lib/db/notion/mapImage'
...
export async function fetchGlobalAllData({
  ...
  const cachedData = await getOrSetDataWithCache(cacheKey, async () => {
    ...
    try {
      ...
      // 拉取失败必须向上抛:吞掉异常会把 EmptyData 当正常结果写进缓存,
      // 导致限流/网络抖动期间全站白屏且需重启才能恢复
...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  if (!post && typeof lastSegment === 'string' && /^[a-f0-9-]{32,36}$/i.test(lastSegment)) {
    ...
    try {
      ...
      // 拉取失败(限流/网络)必须向上抛:吞掉会让 notFound 被 ISR 缓存,
      // 瞬时故障变成持续整个 revalidate 周期的 404
  ...
  if (post) {
    ...
    try {
      ...
      // processPostData 中断时 prev/next/recommendPosts 可能未被赋值；
      // 补齐可序列化默认值，避免 getStaticProps 返回 undefined 导致整个构建失败
...
async function convertNotionToSiteData(
  ...
  if (!pageRecordMap) {
    // 拉取失败(如 Notion 429)时抛错而不是返回 {}:
    // 让缓存层跳过写入,避免空数据被缓存后持续白屏
    throw new Error(
      `[${traceId}] can't get Notion Data ; pageId: ${SITE_DATABASE_PAGE_ID}`
    )
  }
  ...
  // 手动上传的自定义表情图标（notion://custom_emoji/...）真实地址表
  const customEmojiMap = pageRecordMap.custom_emoji || {}
  ...
  for (const id of pageIds) {
    ...
    if (properties) {
      // 自定义表情图标还原为可访问的图片地址
      properties.pageIcon = resolveCustomEmojiIcon(
        properties.pageIcon,
        customEmojiMap
      )
  ...
  // collection 查询与 getBlocks 的 recordMap 均不含 custom_emoji 表，
  // 只有 getPage 的响应里有；对仍是 notion:// 的图标逐页补拉（带缓存），
  // 提取真实地址后再解析（含数据库页自身，用于站点图标）
  const unresolvedEmojiPosts = collectionData.filter(p =>
    p?.pageIcon?.startsWith('notion://custom_emoji/')
  )
  if (
    unresolvedEmojiPosts.length > 0 ||
    collection?.icon?.startsWith('notion://custom_emoji/')
  ) {
    for (const id of [
      SITE_DATABASE_PAGE_ID,
      ...unresolvedEmojiPosts.map(p => p.id)
    ]) {
      try {
        const rm = await getOrSetDataWithCache(
          `notion_custom_emoji_${id}`,
          () => notionAPI.getPage(id)
        )
        if (rm?.custom_emoji) {
          Object.assign(customEmojiMap, rm.custom_emoji)
        }
      } catch (e) {
        console.warn(
          `[${traceId}] 补拉 custom_emoji 失败: ${id} ${e.message}`
        )
      }
    }
    unresolvedEmojiPosts.forEach(p => {
      p.pageIcon = resolveCustomEmojiIcon(p.pageIcon, customEmojiMap)
    })
  }
  ...
  const siteInfo = getSiteInfo({ collection, block, rawMetadata, NOTION_CONFIG, customEmojiMap })
 ...
 */
function getSiteInfo({ collection, block, rawMetadata, NOTION_CONFIG, customEmojiMap }) {
  ...
  let icon = compressImage(
    collection?.icon
      ? mapImgUrl(
          resolveCustomEmojiIcon(collection?.icon, customEmojiMap),
          collection,
          'collection'
        )
      : defaultIcon
