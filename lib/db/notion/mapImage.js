...
/**
 * 解析 Notion 自定义表情（手动上传）图标地址
 * 此类图标在 page_icon 中为 notion://custom_emoji/<spaceId>/<emojiId> 私有协议，
 * 真实图片地址存在于 recordMap 的 custom_emoji 表中；
 * 命中返回可公开访问的图片 URL，未命中返回原值（由渲染层忽略）
 * @param {string} icon
 * @param {object} customEmojiMap recordMap.custom_emoji
 * @returns {string}
 */
const resolveCustomEmojiIcon = (icon, customEmojiMap) => {
  if (
    typeof icon !== 'string' ||
    !icon.startsWith('notion://custom_emoji/')
  ) {
    return icon
  }
  const emojiId = icon.split('/')[4]?.split('?')[0]
  const entry = emojiId && customEmojiMap?.[emojiId]
  // 兼容 value.value.url（原始 recordMap 结构）与 value.url（拍平后结构）
  const url = entry?.value?.value?.url || entry?.value?.url
  return url || icon
}

export { compressImage, mapImgUrl, resolveCustomEmojiIcon }
