 ...
 * @param {*} ids
 * @param {*} batchSize
 * @param {*} emojiSink 可选，传入对象时会把响应 recordMap.custom_emoji 合并进去
 *        （collection 查询结果不含自定义表情表，手动上传图标的真实地址需从此处收集）
 * @returns
 */
export const fetchInBatches = async (ids, batchSize = 30, emojiSink = null) => {
  ...
  for (let i = 0; i < ids.length; i += batchSize) {
    ...
    try {
      const result = await limit(async () => {
        ...
        // 收集本批次的自定义表情图标真实地址
        if (emojiSink && pageChunk?.recordMap?.custom_emoji) {
          Object.assign(emojiSink, pageChunk.recordMap.custom_emoji)
        }
