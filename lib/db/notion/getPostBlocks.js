...
export async function getPageWithRetry(
  ...
  if (!retryAttempts || retryAttempts <= 0) {
    // 重试耗尽:抛错而不是返回 null。
    // 上游据此区分「页面不存在」(null → 404)与「拉取失败」(throw →
    // ISR 保留旧页/返回 500),避免瞬时故障被缓存成 404 或空数据
    console.error('[请求失败]:', `from:${from}`, `id:${id}`)
    throw new Error(`[getPageWithRetry] fetch failed after retries: ${id}`)
  }
  ...
  try {
    ...
    // 页面不存在是确定性结果,直接返回 null(→ 404),不重试
    if (isNotFoundError(e)) {
      return null
    }

    // 优先回退旧缓存(单页级 stale-if-error)
    ...
    // 缓存兜底也没有 → 退避后再重试
    await sleep(backoffDelayMs(retryAttempts))
    return getPageWithRetry(id, from, retryAttempts - 1, cacheKey)
  }
}

// Notion 对不存在/无权限页面的报错视为永久错误(400 / not found)
function isNotFoundError(e) {
  const msg = String(e?.message || e || '')
  return e?.status === 400 || /not found|not exist|could not find/i.test(msg)
}

// 指数退避(0.5s/1s/2s)+ 随机抖动:避免 429 限流期间立即重试、越失败越请求
function backoffDelayMs(retryAttemptsLeft) {
  const attempt = 3 - retryAttemptsLeft // 已失败次数(0 起)
