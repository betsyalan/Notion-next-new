...
function getRawNotion() {
  if (!globalStore.notion) {
    globalStore.notion = new NotionLibrary({
      ...
      // Notion 自 2026-08-05 起封禁 Node 默认 UA(user-agent: node)返回 403,
      // 必须伪装成浏览器 UA 才能正常回源
      ofetchOptions: {
        headers: {
          'user-agent':
      ...
      // 注意:kyOptions 在当前 notion-client(ofetch 版)中已被忽略,下面的 hook 实际不生效,
      // 保留仅为兼容旧版库;endpoint 重写如需生效应迁移到 ofetchOptions.onRequest
      kyOptions: {
        mode: 'cors',
        hooks: {
          beforeRequest: [
