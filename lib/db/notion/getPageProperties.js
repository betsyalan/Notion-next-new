...
export default async function getPageProperties(
  ...
  // slug 列若为公式类型，空结果是 '' 而非 undefined，统一视为未填写，走 ?? 兜底（回退页面 UUID）
  if (properties.slug === '') {
    properties.slug = undefined
  }

  // slug 列为公式类型时：非官方 API 只返回服务端缓存的旧值（实测 queryCollection
  // 与 getPage 均不随公式/行编辑实时刷新），无法依赖；按约定的自动生成规则
  // 'page'+页面id（与公式 "page"+id() 等价）在本地生成
  const slugPropertyName = BLOG.NOTION_PROPERTY_NAME?.slug || 'slug'
  const slugSchemaKey = Object.keys(schema || {}).find(
    k => schema[k]?.name === slugPropertyName
  )
  if (slugSchemaKey && schema[slugSchemaKey]?.type === 'formula') {
    properties.slug = 'page' + id.replace(/-/g, '')
  }
...
/**
 * 从标题自动生成 slug（当 Notion 数据库没有 slug 字段时使用）
 * 将标题转为小写、空格/特殊字符替换为连字符，中文保留原样
 * @param {string} title
 * @returns {string}
 */
